...
```
  設計層     Architect
             ↑ 設計判断がコードに反映され、構造として残る
  ───────────────────────────
  安定化層   Anchor / Cleaner
             ↑ 品質を保ち、負債を片付け、コードが長く生き残る
  ───────────────────────────
  実装層     Producer / Growing
             コードを書いて出荷する
```
...
```
  設計層     → Former（退任後、設計知識がチームから消える）
  安定化層   → Silent（貢献が止まる）、Fragile（放置されたコードだけ残る）
  実装層     → Silent（書かなくなる）
```
...
しかし、もしそれが可能になれば、現在のような単純なLegacy-Heavy警告ではなく、**「履歴として重いだけの健全な構造」と「本当に危険な依存構造」**を区別できるようになるはずだ。それはEISの次のフロンティアかもしれない。
