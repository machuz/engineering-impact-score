...
冷たいgit履歴から、こういう**チームの物語**が読み取れる。そしてモジュールトポロジーは「誰が強いか」だけでなく、**「どこが壊れかけているか」**を教えてくれる。
...
本当の意味での「優秀さ」は、**複数の宇宙に残した痕跡**によって初めて定量化できる。一つのコードベースでのスコアはローカルな観測に過ぎない。異なるコードベース、異なるチーム、異なるドメインで一貫して高いスコアを残せるなら——それは再現性のある重力だ。一つの銀河で輝く星と、自然の力そのものの違い。

いくつか大事なことを書いておく。

**EISが測っているのは「このコードベースでのインパクト」であって、エンジニアとしての絶対的な能力ではない。** スコア40は「このコードベースにおいて、その人のコードが残り、設計に影響を与え、負債を掃除している」ということ。別のコードベースに移れば、スコアは逆転するかもしれない。（これをエンジニアリング相対性理論と呼んでいる。）

**コンテキストなしのスコアは危険。** Survivalが低いのは設計が悪いからかもしれないし、レガシーコードを書き換えている最中（Rescueスタイル）だからかもしれない。設計の悪いコードベースでスコアが高いのは「誰もリファクタできないコード」を書いているだけかもしれない。常にコンテキストとセットで解釈すべきだ。
...
**時間減衰がゲーミングを防ぐ。** 忙しそうに見せる作業ではスコアは上がらない——数ヶ月後にコードベースに残っているコードだけがカウントされる。負債清掃軸があることで、他者に仕事を発生させてスコアを上げることも構造的にできない。
...
1. **[履歴だけでエンジニアの「戦闘力」を定量化する](https://zenn.dev/machuz/books/git-archaeology/viewer/ch1)** — 7軸スコアリングの全設計
...
5. **[タイムライン：スコアは嘘をつかないし、遠慮も映る](https://zenn.dev/machuz/books/git-archaeology/viewer/ch5)**
...
8. **[Engineering Relativity：なぜ同じエンジニアでもスコアが変わるのか](https://zenn.dev/machuz/books/git-archaeology/viewer/ch8)**
